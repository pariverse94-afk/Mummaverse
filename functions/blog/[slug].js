import { getPost, listPosts, renderMarkdown, escapeHtml, html, imageUrl } from '../_lib.js';
...
export async function onRequestGet(context) {
  ...
  <main class="wrap" style="padding-top:48px;">
    ...
    <article>
      ...
      ${post.hasImage ? `<img src="${escapeHtml(imageUrl(post))}" alt="${escapeHtml(post.title)}" style="width:100%; border-radius:16px; margin:28px 0 0; display:block;">` : ''}
  ...
  return html(page({
    ...
    ogImage: post.hasImage ? `${SITE.origin}${imageUrl(post)}` : undefined,
    body,
    jsonLd: {
