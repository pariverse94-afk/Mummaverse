  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between h-16 md:h-20">
            <button onClick={onLogoClick} className="flex items-center gap-2.5 anim-in" aria-label="Home">
              ...
              <div className="flex flex-col leading-tight">
                <span className="font-oswald text-lg font-500 tracking-wide text-[#2C1810]">Mummaverse</span>
      ...
      <div className={`mobile-menu fixed inset-0 z-40 bg-[#FDF8F3]/95 backdrop-blur-xl flex flex-col items-center justify-center gap-8 ${menuOpen ? 'open' : ''}`} role="dialog">
        {[['problem','Why Pariverse'],['features','Features'],['village','The Village'],['blog','Blog'],['faq','FAQ']].map(([id, label]) => (
          <button key={id} onClick={() => scrollTo(id)} className="text-2xl font-oswald uppercase tracking-wider text-[#2C1810] hover:text-[#E8600C] transition-colors">
            {label}
          </button>
        ))}
